...
class Kconfig(object):
    ...
    def write_config_generic(self, filename, config_to_str, comment_to_str, header, footer):
        r"""
        Writes out symbol values in the ... format.
            ...
            while 1:
                ...
                if isinstance(item, Symbol):
                    if not item._written:
                        ...
                        config_string = config_to_str(item)
                        if config_string:
                            write(config_string)
                        item._written = True
                    ...
                    write(comment_to_str(node.prompt[0]))
    ...
    def write_config_ada(self, filename, header="--  Generated by Kconfiglib (https://github.com/ulfalizer/Kconfiglib)\n"):
        ...
        def config_to_ada(item):
            ...
            if item.type == BOOL:
                ...
                _internal_error("Internal error while creating config.ads: unknown "
                                'type "{0}:{1}".'.format(item.type, item.name))

        def comment_to_ada(str):
            return "\n   --\n   --  %s\n   --\n" % str

        self.write_config_generic(filename,
                                  config_to_str=config_to_ada,
                                  comment_to_str=comment_to_ada,
                                  header=ada_header,
                                  footer=ada_footer)

    def write_config_gpr(self, filename, header="--  Generated by Kconfiglib (https://github.com/ulfalizer/Kconfiglib)\n"):
        ...
        def config_to_gpr(item):
            ...
            if item.type == BOOL:
                ...
                if item.source_dirs and val == "True":
                    src_dirs='   Src_Dirs := Src_Dirs & ("%s");\n' % ('", "'.join(item.source_dirs))

                return ('   ' + item.kconfig.config_prefix +
                        '{0} : Boolean := "{1}";\n{2}'.format(item.name, val, src_dirs))
            ...
            elif item.type == STRING or item.type == INT or item.type == HEX:
                # Escape \ and "
                return ('   ' + item.kconfig.config_prefix +
                          '{0} := "{1}";\n'.format
                          (item.name,
        ...
        def comment_to_gpr(str):
            return "\n   --\n   --  %s\n   --\n" % str

        self.write_config_generic(filename,
                                  config_to_str=config_to_gpr,
                                  comment_to_str=comment_to_gpr,
                                  header=gpr_header,
                                  footer=gpr_footer)
